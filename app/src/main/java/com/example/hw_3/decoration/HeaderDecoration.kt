package com.example.hw_3.decoration
...
import com.example.hw_3.JobListAdapter
...
class HeaderDecoration: RecyclerView.ItemDecoration() {
    ...
    override fun onDrawOver(c: Canvas, parent: RecyclerView, state: RecyclerView.State) {
        ...
        val bitmapOffSet =
            if (viewY > 0 && viewY <= bitmapHeight) viewY - bitmapHeight
            else 0f

        viewHolder?.itemView?.alpha = if (viewY < 0) 0f else 1f
